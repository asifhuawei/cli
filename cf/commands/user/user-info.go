...
func ShowUserInfo(ui terminal.UI, config core_config.Reader, userRepo api.UserRepository, orgRepo organizations.OrganizationRepository, spaceRepo spaces.SpaceRepository) (cmd *CFUsers) {
	cmd = new(CFUsers)
	cmd.ui = ui
	cmd.config = config
...
func (cmd *CFUsers) Run(c *cli.Context) {
	...
	for apiName, displayName := range models.ApiToOrgRole {
		orgs, apiErr := cmd.orgRepo.GetRole(apiName, orgName)
	...
	for apiName, displayName := range models.ApiToSpaceRole {
